...
extern char status_door ;
extern char prev_status_door;
extern u32 timedoor;
...
char User_commands(STRUCTCOMMAND_SIM900 cmd_data)
{
   ...
   {  
      ...
      {
         SentEnglis_SIMmsg(flashv.user[0].PHONE_NO,"Danh ba chi ton tai duy nhat so ADMIN\n");  
   ...
   else if(!strcmp(p,"MOCUA"))
   {
	  if(SearchExistUser(decodeSMS.numberPhone) == 10)
      {
		...
		SentEnglis_SIMmsg(decodeSMS.numberPhone,"Cua vua duoc mo\r");
		return 1;
   }	 
   else if(!strcmp(p,"DONGCUA"))
   {
 	  if(SearchExistUser(decodeSMS.numberPhone) == 10)
      {
         return 1;
		...
		timedoor 			= sysTick_counter; 
		SentEnglis_SIMmsg(decodeSMS.numberPhone,"Cua vua duoc dong\r");
		return 1;
   }	 	 
   else if(!strcmp(p,"DUNGCUA"))
   {
   	  if(SearchExistUser(decodeSMS.numberPhone) == 10)
      {
         return 1;
		...
		DoorControl(STOP);
		if(prev_status_door == CLOSE) prev_status_door = CLOSSING;//OPENNING ; 
		if(prev_status_door == OPEN)  prev_status_door = OPENNING;//CLOSSING ;
		status_door  		= STOPED;	 
		SentEnglis_SIMmsg(decodeSMS.numberPhone,"Cua vua duoc dung\r");
		return 1;
   }	 	 	 	 
   else
   {
...
/*******************************************************************************
* Function Name  : DieuKhienCua
* Description    : Dieu Khien cua dung
* Input          : None
* Output         : None
* Return         : None
* Attention		 : None
