...
OSA_TASK_DEFINE(service_task, SERVICE_TASK_STACK_SIZE);
...
int main(void)
{
    ...
    OSA_Init();
    /* Create print tasks */
    result = OSA_TaskCreate( service_task,
                          "service task",
                          SERVICE_TASK_STACK_SIZE,
                          service_task_stack,
                          SERVICE_TASK_PRIORITY_OSA,
                          (task_param_t)0,
                          false,
                          &service_task_task_handler
                        );
...
void service_task ( task_param_t init_data )
{
    ...
    result = OSA_TaskCreate( simulated_isr_task,
                            ...
                            simulated_isr_task_stack,
                            SIMULATED_ISR_TASK_PRIORITY_OSA,
                            (task_param_t)(0),
                            false,
                            &simulated_isr_task_task_handler
                        );
